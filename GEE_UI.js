//VolcanoResponse GEE script file

// rood directory for assets 
var root_dir = 'users/armds'

// import list of eruptions from 2020
var table = ee.FeatureCollection(root_dir + "/SampleEruptions");

// Import damage files from models
var damage_areas = ee.FeatureCollection(root_dir + "/all_data2");
var sakura = ee.FeatureCollection(root_dir + "/fake_sakura")
damage_areas = damage_areas.merge(sakura)

// Import cities by volcanoes
var locations_dev = ee.FeatureCollection(root_dir + "/volcano_cities");
var cities = ee.FeatureCollection(root_dir + "/city_data");

print(cities)

// Import power plant info
var plants = ee.FeatureCollection(root_dir + "/volc_power_plant");

print(plants)
// Import Dam info
var dams = ee.FeatureCollection(root_dir + "/volc_dam");

print(dams)
// Get the list of volcano names of the ones we have models for
var volcanos = table.filter(ee.Filter.eq('all_data', true)).aggregate_array('VolcanoName').distinct().sort()
  ...
  onChange: function() {
    ...
    var select_date = ui.Select({
      ...
      onChange: function() {
        ...
        var subtitle = ui.Label(' Select a volcano that you would like to see the damaged area. ', 
                                {fontWeight: 'bold', fontSize: '18px', margin: '10px 5px',backgroundColor:'lightgray'});
        var sub2 = ui.Label(' What date would you like to view the damage area? ', 
                                {fontWeight: 'bold', fontSize: '18px', margin: '10px 5px',backgroundColor:'lightgray'});
        ...
        // Add damage polygons
        var damage_254_all = damage.filter(ee.Filter.eq('layer',254.0))
        var damage_254 = damage_254_all.filter(ee.Filter.eq('diffdiff_d',date))
        var damage_255_all = damage.filter(ee.Filter.eq('layer',255.0))
        var damage_255 = damage_255_all.filter(ee.Filter.eq('diffdiff_d',date))
        var damage_area = damage_254.geometry().area({'maxError': 1}).getInfo(0);
        var damage_area = ee.Number(damage_area).divide(ee.Number(1000000)).round().getInfo(0);
        

        var subtitle1 = ui.Label('- Highly Impacted Area: '+ damage_area + ' km2', 
                                 {fontWeight: 'bold', fontSize: '18px', margin: '10px 5px',backgroundColor:'lightgray'});
        var maincityheader = ui.Label('Closest Cities', 
                                 {fontWeight: 'bold', fontSize: '20px', margin: '10px 5px',backgroundColor:'lightgray'});
        
        Map.addLayer(damage_255,{'color': 'yellow'},"Minor Damage")
        Map.addLayer(damage_254,{'color': 'red'},"Severe Damage")        
        

        panel.add(header).add(subtitle).add(select_volcano).add(sub2)
        panel.add(select_date).add(subheader).add(subtitle1).add(maincityheader);
        Map.add(legend);
        ...
        while ( i < sz.getInfo()){ 
          //print("processing: ", i)
          var cityheader = ui.Label(where.get(i).getInfo() + '', 
                                   {fontWeight: 'bold', fontSize: '17px', margin: '2px 5px',backgroundColor:'lightgray'});
          panel.add(cityheader)
          var citypop = ui.Label('- Population: ' + pop.get(i).getInfo(0), 
                                   {fontSize: '16px', margin: '2px 5px',backgroundColor:'lightgray'});
          panel.add(citypop)
          var citydist = ui.Label('- Distance: ' + dist.get(i).getInfo(0) + ' km', 
                                   {fontSize: '16px', margin: '2px 5px',backgroundColor:'lightgray'});
          panel.add(citydist)
          i++
        }
        ...
        var mainpowerheader = ui.Label('Closest Power Plants', 
                                       {fontWeight: 'bold', fontSize: '20px', margin: '15px 5px',backgroundColor:'lightgray'});
        ...
        while ( i < szp.getInfo()){ 
          var powerheader = ui.Label('Plant Name: ' + name.get(i).getInfo() + '', 
                                     {fontWeight: 'bold', fontSize: '17px', margin: '2px 5px',backgroundColor:'lightgray'});
          panel.add(powerheader)
          var powerdist = ui.Label('- Distance: ' +dist.get(i).getInfo(0) + ' km', 
                                     {fontSize: '16px', margin: '2px 5px',backgroundColor:'lightgray'});
          panel.add(powerdist)
          var powertype = ui.Label('- Fuel: ' + type.get(i).getInfo(0), 
                                     {fontSize: '16px', margin: '2px 5px',backgroundColor:'lightgray'});
          panel.add(powertype)
          var powersize = ui.Label('- Size: ' + size.get(i).getInfo(0) + ' MW', 
                                    {fontSize: '16px', margin: '2px 5px',backgroundColor:'lightgray'});
          panel.add(powersize)
          i++
        }      
        ...
        var maindamheader = ui.Label('Closest Dams', 
                                     {fontWeight: 'bold', fontSize: '20px', margin: '15px 5px',backgroundColor:'lightgray'});
        ...
        while ( i < szd.getInfo()){ 
          //print("processing: ", i)
          var damheader = ui.Label('Dam ID: ' + name.get(i).getInfo() + '', 
                                   {fontWeight: 'bold', fontSize: '17px', margin: '2px 5px',backgroundColor:'lightgray'});
          panel.add(damheader)
          var damdist = ui.Label('- Distance: ' + dist.get(i).getInfo(0) + ' km', 
                                   {fontSize: '16px', margin: '2px 5px',backgroundColor:'lightgray'});
          panel.add(damdist)
          i++
        }        
